...
class PPTPTunnelManager:
 ...
 def create_tunnel(self,node_id,node_ip,username,password):
  try:
   ...
   logger.info(f"PPTP PID {p.pid}")
   for i in range(15):
    time.sleep(1)
    if p.poll()is not None:logger.error(f"died {p.poll()}");return None
    r=subprocess.run("ip a|grep 'ppp[0-9]'",shell=True,capture_output=True,text=True)
    m=re.findall(r'(\d+):\s+(ppp\d+):',r.stdout)
    if m:
     m.sort(key=lambda x:int(x[0]));iface=m[-1][1]
     ipr=subprocess.run(f"ip a s {iface}",shell=True,capture_output=True,text=True)
     lm=re.search(r'inet\s+([\d.]+)',ipr.stdout);rm=re.search(r'peer\s+([\d.]+)',ipr.stdout)
     if lm and rm:
      ...
      try:
       ...
       if n:n.ppp_interface=iface;db.commit()
       db.close()
      except:pass
      logger.info(f"OK: {iface}")
      return ti
   return None
  except:return None
 def destroy_tunnel(self,nid):
  try:
   if nid in self.active_tunnels:subprocess.run(f"kill -9 {self.active_tunnels[nid]['pid']}",shell=True);del self.active_tunnels[nid]
   return True
  except:return False
