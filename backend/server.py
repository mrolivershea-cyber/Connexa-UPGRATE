...
async def manual_speed_test_batch_progress(
    ...
    node_ids_to_test = test_request.node_ids or []
    if not node_ids_to_test:
        logger.info("🌐 SPEED TEST BATCH: Select All mode detected - loading all nodes from database")
        all_nodes = db.query(Node).all()
        node_ids_to_test = [node.id for node in all_nodes]
        logger.info(f"📊 SPEED TEST BATCH: Will test {len(node_ids_to_test)} nodes (all nodes in database)")
