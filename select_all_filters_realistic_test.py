...
class SelectAllFiltersRealisticTester:
    def __init__(self, base_url="https://speedtest-recovery.preview.emergentagent.com"):
    ...
    def test_select_all_filters_scenario_1_ping_light_status_filter(self):
        ...
        # Allow small discrepancy due to concurrent operations
        if abs(total_filtered - not_tested_count) > 100:  # Allow up to 100 difference
            self.log_test("Select All Filters - PING LIGHT Status Filter", False, 
                         f"Large mismatch: count API returned {not_tested_count}, all-ids returned {total_filtered}")
            return False
        
        # Test PING LIGHT with filters (using batch endpoint) - limit to first 10 nodes for testing
        test_data = {
        ...
        if ping_success:
            ...
            if session_id:
                ...
                time.sleep(3)  # Let it start processing
                ...
                if progress_success:
                    ...
                    # Verify that total_items matches our filtered count (approximately)
                    if abs(total_items - total_filtered) <= 100:  # Allow small discrepancy
                        self.log_test("Select All Filters - PING LIGHT Status Filter", True, 
                                     f"PING LIGHT with status=not_tested filter working: {total_items} nodes being tested (matches filtered count ±100)")
                        return True
                    else:
                        self.log_test("Select All Filters - PING LIGHT Status Filter", False, 
    ...
    def test_select_all_filters_scenario_2_ping_ok_protocol_filter(self):
        ...
        # Get count of nodes with protocol=pptp (lowercase, as shown in stats)
                ...
                if progress_success:
                    ...
                    if abs(total_items - total_filtered) <= 100:  # Allow small discrepancy
                        self.log_test("Select All Filters - PING OK Protocol Filter", True, 
                                     f"PING OK with protocol=pptp filter working: {total_items} nodes being tested (matches filtered count ±100)")
                        return True
                    else:
                        self.log_test("Select All Filters - PING OK Protocol Filter", False, 
                                     f"Filter not applied correctly: expected ~{total_filtered} nodes, got {total_items}")
    ...
    def test_select_all_filters_scenario_3_socks_start_status_filter(self):
        """СЦЕНАРИЙ 3: SOCKS Start с фильтром status=ping_light + Select All (using available status)"""
        print(f"\n🔥 TESTING SELECT ALL WITH FILTERS - SCENARIO 3: SOCKS Start with status=ping_light filter")
        
        # Get count of nodes with status=ping_light (available status)
        count_success, count_response = self.make_request('GET', 'nodes/count?status=ping_light')
        
        if not count_success:
            self.log_test("Select All Filters - SOCKS Start Status Filter", False, 
                         f"Could not get count of ping_light nodes: {count_response}")
            return False
        
        ping_light_count = count_response.get('count', 0)
        print(f"📊 Found {ping_light_count} nodes with status=ping_light")
        
        if ping_light_count == 0:
            self.log_test("Select All Filters - SOCKS Start Status Filter", False, 
                         f"No nodes with status=ping_light found for testing")
            return False
        
        # Get all node IDs with status=ping_light (Select All functionality)
        ids_success, ids_response = self.make_request('GET', 'nodes/all-ids?status=ping_light')
        ...
        if socks_success:
            ...
            # SOCKS start should reject ping_light nodes (they need ping_ok or speed_ok)
            if started_count == 0:
                self.log_test("Select All Filters - SOCKS Start Status Filter", True, 
                             f"SOCKS Start correctly rejected {total_filtered} ping_light nodes (need ping_ok/speed_ok status)")
                return True
            else:
                self.log_test("Select All Filters - SOCKS Start Status Filter", False, 
                             f"Unexpected SOCKS start count: {started_count} (expected 0 for ping_light nodes)")
                return False
        else:
    ...
    def test_select_all_filters_scenario_4_socks_stop_status_filter(self):
        """СЦЕНАРИЙ 4: SOCKS Stop с фильтром status=ping_failed + Select All (using available status)"""
        print(f"\n🔥 TESTING SELECT ALL WITH FILTERS - SCENARIO 4: SOCKS Stop with status=ping_failed filter")
        
        # Get count of nodes with status=ping_failed (available status)
        count_success, count_response = self.make_request('GET', 'nodes/count?status=ping_failed')
        
        if not count_success:
            self.log_test("Select All Filters - SOCKS Stop Status Filter", False, 
                         f"Could not get count of ping_failed nodes: {count_response}")
            return False
        
        ping_failed_count = count_response.get('count', 0)
        print(f"📊 Found {ping_failed_count} nodes with status=ping_failed")
        
        if ping_failed_count == 0:
            self.log_test("Select All Filters - SOCKS Stop Status Filter", False, 
                         f"No nodes with status=ping_failed found for testing")
            return False
        
        # Get all node IDs with status=ping_failed (Select All functionality)
        ids_success, ids_response = self.make_request('GET', 'nodes/all-ids?status=ping_failed')
        ...
        if socks_success:
            ...
            # SOCKS stop should handle ping_failed nodes gracefully (no SOCKS to stop)
            if stopped_count == 0:
                self.log_test("Select All Filters - SOCKS Stop Status Filter", True, 
                             f"SOCKS Stop correctly handled {total_filtered} ping_failed nodes (no SOCKS services to stop)")
                return True
            else:
                self.log_test("Select All Filters - SOCKS Stop Status Filter", False, 
                             f"Unexpected SOCKS stop count: {stopped_count} (expected 0 for ping_failed nodes)")
                return False
        else:
    ...
    def run_select_all_filters_tests(self):
        ...
        print("🚀 Starting SELECT ALL WITH FILTERS Realistic Testing...")
        ...
        print(f"   not_tested: {stats.get('not_tested', 0)}")
        print(f"   ping_light: {stats.get('ping_light', 0)}")
        print(f"   ping_failed: {stats.get('ping_failed', 0)}")
        print(f"   ping_ok: {stats.get('ping_ok', 0)}")
        print(f"   Protocols: {stats.get('by_protocol', {})}")
        
        print("\n" + "🔥" * 80)
        print("🇷🇺 COMPREHENSIVE TESTING: Select All with Filters для Testing и SOCKS")
        print("🔥" * 80)
        print("ТЕСТОВЫЕ СЦЕНАРИИ (ADAPTED FOR CURRENT DATA):")
        ...
        print("СЦЕНАРИЙ 3: SOCKS Start с фильтром status=ping_light (should reject)")
        print("СЦЕНАРИЙ 4: SOCKS Stop с фильтром status=ping_failed (should handle gracefully)")
...
if __name__ == "__main__":
    tester = SelectAllFiltersRealisticTester()
